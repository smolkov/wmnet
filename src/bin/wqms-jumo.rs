...
///Jumo command argument
#[derive(Debug, StructOpt)]
#[structopt(name = "jumo", about = "🧰jumo interface usage.")]
pub struct Args {
    ///🗁 simulate jumo measurement
...
pub fn simulate() -> (f32, f32, f32, f32) {
    (50.0, 120.0, 7.0, 29.9)
}
...
fn main(args: Args) -> Result<()> {
    ...
    let jumo_do = channels.new_channel("do", "mh/l")?;
    ...
    loop {
        select! {
            recv(ticks) -> _ => {
                if args.debug() {
                    let (ph_res,ec_res,orp_res,do_res) = simulate();
                        jumo_ph.push_data(ph_res)?;
                        jumo_ec.push_data(ec_res)?;
                        jumo_orp.push_data(orp_res)?;
                        jumo_do.push_data(do_res)?;
                } else {
